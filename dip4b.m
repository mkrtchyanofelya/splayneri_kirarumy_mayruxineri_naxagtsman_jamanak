...
N1 = 900;
theta1 = theta1_start + (0:N1-1)*phi1/(N1-1);   
...
N2 = 1000;
theta2 = theta2_start + (0:N2-1)*phi2/(N2-1);   
...
plot(xK,  yK,  'b', 'LineWidth', 2.5);
plot(xA2, yA2, 'r', 'LineWidth', 2.5);

xlabel('x', 'FontSize', 12);
ylabel('y', 'FontSize', 12);
title('Աղեղ -> K-աձև կոր -> Աղեղ', 'FontSize', 14);
...
disp(['alpha2 (deg) = ', num2str(alpha2*180/pi)]);
disp(['R_check = ', num2str(R_check)]);
disp(['L_check = ', num2str(L_check)]);
disp(['Start arc center = (', num2str(xc1), ', ', num2str(yc1), ')']);
disp(['End arc center = (', num2str(xc2), ', ', num2str(yc2), ')']);
